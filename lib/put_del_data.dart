...
class PutDelData extends StatefulWidget {
  const PutDelData({Key? key}) : super(key: key);

  @override
  _PutDelDataState createState() => _PutDelDataState();
}

class _PutDelDataState extends State<PutDelData> {
  ...
  Widget build(BuildContext context) {
    ...
    _DocumentNoController.text = args[1].toString();
    var tanggal = f.format(DateTime.parse(args[2])).toString();
    _DocumentDateController.text = f.format(DateTime.parse(args[2])).toString();
    _CostNameController.text = args[3];
    _CostSourceController.text = args[4];
    _DivisionCodeController.text = args[5];
    _QtyController.text = args[6].toString();
    _UnitController.text = args[7];
    _PriceController.text = args[8].toString();
    _AmountController.text = (args[6] * args[8]).toString();
    _RemarksController.text = args[13];
    _Image01Controller.text = args[10];
    /*_FixClsController.text = args[14].toString();
    _FixDateController.text = args[15];
    _FixUserController.text = args[16];
    _UpdateDateController.text = args[17];
    _UpdateUserController.text = args[18];
    _EntryDateController.text = args[19];
    _EntryUserController.text = args[20];
    if (args[10].isNotEmpty) {
      _Image01Controller.text = args[10];
    }
    if (args[11].isNotEmpty) {
      _Image02Controller.text = args[11];
    }
    if (args[12].isNotEmpty) {
      _Image03Controller.text = args[12];
    } */
                                    ...
                                    child: Column(
                                      children: <Widget>[
                                        SfDateRangePicker(
                                          ...
                                          initialSelectedDate:
                                              DateTime.parse(_DocumentDateController.text),
                                          onSelectionChanged:
                                              _onSelectionChanged,
                                          selectionMode:
                ...
                children: <Widget>[
                  ...
                  Flexible(
                    child: DropdownSearch<String>(
                      ...
                      selectedItem: _CostSourceController.text,
                    ),
                  )
                ],
              ),
                ...
                children: [
                  ...
                  Flexible(
                    child: DropdownSearch<String>(
                      ...
                      selectedItem: _DivisionCodeController.text,
                    ),
                  ),
                ],
              ),
                  ...
                  child: Row(children: <Widget>[
                    Visibility(
                      ...
                      child: Container(
                        height: 150,
                        width: 150,
                        child: Image.memory(base64.decode(args[10])),//args[10] == null ? Image.memory(base64.decode(imageBase64)) : Image.memory(base64.decode(args[10])),
              ...
              ElevatedButton(
                onPressed: () async {
                  if (_DocumentNoController.text.isNotEmpty &&
                      ...
                      _CostSourceController.text.isNotEmpty) {
                    bool response = await repository.putData(
                        ...
                        _RemarksController.text, imageBase64);
                    if (response) {
    ...
    return Container(
        ...
        child: Card(
          child: SfDateRangePicker(
            selectionColor: Color.fromRGBO(64, 75, 96, .9),
            todayHighlightColor: Color.fromRGBO(64, 75, 96, .9),
            showTodayButton: true,
            showNavigationArrow: true,
            initialSelectedDate: DateTime.parse(_DocumentDateController.text),
